...
The number of people on Earth is finite, and each of them have a number between 0 and 1 that encodes all of their information. We can then interleave the numbers together to get one number. So using a cut on a stick, we can store all the information you could think of for everyone on Earth.
